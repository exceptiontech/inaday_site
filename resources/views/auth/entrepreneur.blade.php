        ...
        <div class="container">
            <div class="row">
                <div class="col-12 col-md-6 pb-5 pb-sm-0 mb-5 mb-sm-0">
                    ...
                    <p>كثير من رواد الأعمال يأخرون بدء مشاريعهم من أجل مهام ليس لها علاقة حقيقة بنجاح المشروع، فالانتظار
                        أشهر من أجل اختيار اسم أو شعار أو بناء موقع الكتروني، أو انتظار تمويل هي أهم أسباب تعطل كثير من
                        المشاريع وإصابة أصحابها بالإحباط قبل البدء. </p>

                    @guest
                        <a class="btn btn-primary mt-3 mb-3 "
                            href="{{ url('/register?type=entrepreneur') }}">{{ trans('file.register_now') }}</a>
                    @else
                        @if (Auth::user() && Auth::user()->isEntrepreneur())
                            <a class="btn btn-primary  mt-3 mb-5"
                                href="{{ url('/account/profile') }}">{{ __('file.profile') }}</a>
                        @else
                            <a class="btn btn-primary  mt-3 mb-5"
                                href="{{ url('/errors/denied') }}">{{ trans('file.register_now') }}</a>
                        @endif
                    @endguest
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <p>العمل عن بعد هو أسلوب الحياة الجديد، في منصة D24. بتكون تشتغل بالوقت اللي يناسبك، وبالقيمة اللي
                        تناسبك، وفي المكان اللي يناسبك. بيكون عملك فيه إجازة، و إجازتك فيها عمل! ممتع؟ إذا تعتقد انك موهوب،
                        وتحب الشغل، جرب احسب دخلك.</p>

                    @if (Auth::user() && Auth::user()->isEntrepreneur())
                        <a class="btn btn-primary  mt-3 mb-3 "
                            href="{{ url('account/projects/create') }}">{{ __('file.add_newـproject') }}</a>
                    @else
                        <a class="btn btn-primary  mt-3 mb-3  "
                            href="{{ url('/projects') }}">{{ __('file.knowـprojects') }}</a>
                    @endif
