 ...
 --> <!-- <li>
                    ...
                    <div class="col-12 col-md-6 widget">
                        ...
                        <ul class="list-unstyled">
                            @guest
                                <li>
                                    <a
                                        href="{{ url('/register?type=services_provider') }}">{{ trans('file.servives_provider_register') }}</a>
                                </li>
                                <li>
                                    <a
                                        href="{{ url('/register?type=entrepreneur') }}">{{ trans('file.entrepreneur_register') }}</a>
                                </li>
                            @endguest
                            <li>
                        ...
                        <div class="col-12 col-md-9">
                            <div class="row">
                                <div class="col-7 col-md-8  p-0">
                                    <input required="required" class="form-control h-100" id="search"
                                        placeholder="{{ trans('file.email') }}" name="email" type="text">
                                </div>
                                <div class="col-5 col-md-4 p-0 mr-n1">
                                    <button id="subscribe" class="btn btn-block btn-primary h-100"
                                        type="submit">{{ trans('file.subscribe_now') }}</button>
                                </div>
    ...
    <div class="copyrights pb-2 pt-3">
        <div class="container">
            <div class="row">
                <div class="col-12 col-sm-6">
                    <p class="copyright font-weight-bolder mb-2">© 2020 {{ trans('file.byـd24ـteam') }}</p>
...
@guest
                ...
                <div class="modal-body">
                    ...
                    <form method="post" name="login-form" class="login-form" id="login">
                        ...
                        <div class="form-group text-left">
                            ...
                            <input type="email" name="email" id="email" id="login-form-username"
                                class="form-control d-block  @error('email') is-invalid @enderror"
    ...
    @enderror
         -->
                        ...
                        <div class="form-group text-left">
                            ...
                            <input type="password" name="password" id="password" id="login-form-password"
                                class="form-control d-block @error('password') is-invalid @enderror" required
    ...
    @enderror -->
                        ...
                        <button class="btn btn-modal text-center mt-4 btn-login" name="btn-login" id="login-form-submit"
                            value="login">{{ trans('file.signin') }}</button>
                        <h5 class="or">أو</h5>
                        ...
                        <p class="paragrapgh-login">هذه الخاصية للأعضاء المسجلين بالفعل. في حالة التسجيل يرجى استخدام صفحات
                            التسجيل بالأعلى . في
                            حالة الدخول من خلال Google هنا ستكون صاحب عضوية بلا أي صلاحيات </p>
            ...
            function submitForm() {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        $(".error").fadeOut();
                        $(".btn-login").html(
                            '<i class="icon-danger" ></i> {{ trans('file.processing_is_in_progress') }} '
                        );
                    },
                    success: function(data) {
                        ...
                        $(".error").fadeIn(2000, function() {
                            $(".error").html(
                                '<div class="alert alert-success"> <span class="glyphicon glyphicon-info-sign"></span> {{ trans('file.sign_in_successful') }} </div>'
                            );
                        });
                        $(".btn-login").html(
                            '<img src="{{ url('btn-ajax-loader.gif') }}" /> {{ trans('file.signing_in') }} '
                        );
                    ...
                    error: function(jqXHR) {
                        ...
                        $(".error").fadeIn(1000, function() {
                            $(".error").html(
                                '<div class="alert alert-danger"> <span class="glyphicon glyphicon-info-sign"></span> {{ trans('file.sorry_the_email_or_password_are_incorrect') }}  </div>'
                            );
                            $(".btn-login").html(
                                '<i class="icon-signin"></i> {{ trans('file.signin') }} ');
                        });
                    }
                });
                return false;
                ...
                success: function(data) {
                    ...
                    if (data.result == 'done') {
                        $('#subscribe-form .result').html(
                            '<p class="text-success">تم الاشتراك بنجاح</p>');
                    } else {
                        $('#subscribe-form .result').html(
                            '<p class="text-danger">البريد المدخل غير صحيح</p>');
                    }
                },
                error: function(data) {
