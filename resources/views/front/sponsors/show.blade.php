  ...
  <div class="container">
    <div class="row">
      <div class="col-sm-8 blogs-in">
        <div class="blog-det">
          <h3 class="title">{{ $article->title[App::getLocale()]}}</h3><span class="calendar"> <i class="far fa-calendar-alt"></i>{{ $article->created_at}}</span>
          <div class="photo"><img src="{{$article->image}}" alt="{{ $article->getTitle()}}" title="{{ $article->getTitle()}}"></div>
          <p class="text">{!! $article->desc[App::getLocale()] !!}</p>
            ...
            @if (count($random))
                @foreach ($random as $random_article)
                    <a href="{{url('/blog/'.$article->slug)}}">
                      <div class="imgblog"> <img src="{{$random_article->image}}" alt="{{$random_article->getTitle()}}" title="{{$random_article->title[App::getLocale()]}}"></div>
                      <div class="textcontant">
                        <h3 class="title">{{$random_article->title[App::getLocale()]}}</h3><span class="calendar"> <i class="far fa-calendar-alt"></i> {{$random_article->created_at}}</span>
                      </div>
                    </a>
                @endforeach
            @endif
