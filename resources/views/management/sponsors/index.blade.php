...
<div class="col-md-12">
    ...
    <div class="box box-warning">
        <div class="box-header with-border mb-3">
            <h3 class="box-title">
              {{trans('admin.sponsors')}}
              @can('sponsor-create')
              <span class="float-right">
                <a class="btn btn-primary" href="{{ url('/admin/sponsors/create') }}">{{trans('admin.addsponsor')}}</a>
              </span>
              @endcan
            </h3>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @if(count($sponsors))
                            ...
                            <tr>
                                <td>{{$sponsor->id}}</td>
                                <td>{{@$sponsor->title[App::getLocale()] }}</td>
                                <td>{{$sponsor->slug}}</td>
                                <td>
                                ...
                                <td class="actions" width="200">
                                  <a class="btn btn-info" href="{{ action('Admin\SponsorController@edit',$sponsor->id) }}" data-toggle="tooltip" title="{{trans('admin.edit')}}">
                                    {{trans('admin.edit')}}
                                  </a>
                                  {{ Form::open(array('url' => 'admin/sponsors/' . $sponsor->id,'style'=>'display:inline')) }}
