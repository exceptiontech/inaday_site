...
<div class="col-md-12">
    ...
    <div class="box box-warning">
        <div class="box-header with-border mb-3">
            <h3 class="box-title">
              {{trans('admin.costkinds')}}

              @can('costkind-create')
              <span class="float-right">
                <a class="btn btn-primary" href="{{ url('/admin/costkinds/create') }}">{{trans('admin.addcostkind')}}</a>
              </span>
              @endcan
            </h3>
        </div>
        ...
        <div class="box-body">
            ...
            @if (Session::has('message'))
              <div class="alert alert-dismissible alert-{{Session::get('costkind')}}">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @if(count($costkinds))
                            ...
                            <tr>
                                <td>{{$costkind->id}}</td>
                                <td>{{@$costkind->title[App::getLocale()] }}</td>
                                <td>{{$costkind->slug}}</td>
                                <td>
                                      @if($costkind->is_active == 1)
                                ...
                                <td class="actions" width="200">
                                  <a class="btn btn-info" href="{{ action('Admin\CostkindController@edit',$costkind->id) }}" data-toggle="tooltip" title="{{trans('admin.edit')}}">
                                    {{trans('admin.edit')}}
                                  </a>
                                  {{ Form::open(array('url' => 'admin/costkinds/' . $costkind->id,'style'=>'display:inline')) }}
