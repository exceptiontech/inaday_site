        ...
        <div class="box-header with-border mb-3">
            <h3 class="box-title">
              ...
              <span class="float-right">
                <a class="btn btn-primary" href="{{ url('/admin/countries/create') }}">{{trans('admin.addcountry')}}</a>
              </span>
              @endcan
            </h3>
        </div>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @if(count($countries))
                            ...
                            <tr>
                                <td>{{$country->id}}</td>
                                <td>{{@$country->title[App::getLocale()] }}</td>
                                <td>{{$country->slug}}</td>
                                <td>
                                ...
                                <td class="actions" width="200">
                                  <a class="btn btn-info" href="{{ action('Admin\CountryController@edit',$country->id) }}" data-toggle="tooltip" title="{{trans('admin.edit')}}">
                                    {{trans('admin.edit')}}
                                  </a>
                                  {{ Form::open(array('url' => 'admin/countries/' . $country->id,'style'=>'display:inline')) }}
