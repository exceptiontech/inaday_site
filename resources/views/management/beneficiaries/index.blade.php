...
<div class="col-md-12">
    ...
    <div class="box box-warning">
        <div class="box-header with-border mb-3">
            <h3 class="box-title">
              {{trans('admin.beneficiaries')}}

              @can('beneficiary-create')
              <span class="float-right">
                <a class="btn btn-primary" href="{{ url('/admin/beneficiaries/create') }}">{{trans('admin.addbeneficiary')}}</a>
              </span>
              @endcan
            </h3>
        </div>
                    ...
                    <tbody>
                        @if(count($beneficiaries))
                            ...
                            <tr>
                                ...
                                <td>{{@$beneficiary->title[App::getLocale()] }}</td>
                                <td>{{$beneficiary->slug}}</td>
                                <td>
                                ...
                                <td class="actions" width="200">
                                  <a class="btn btn-info" href="{{ action('Admin\BeneficiaryController@edit',$beneficiary->id) }}" data-toggle="tooltip" title="{{trans('admin.edit')}}">
                                    {{trans('admin.edit')}}
                                  </a>
                                  {{ Form::open(array('url' => 'admin/beneficiaries/' . $beneficiary->id,'style'=>'display:inline')) }}
