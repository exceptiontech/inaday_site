...
<div class="col-md-12">
    ...
    <div class="box box-warning">
        <div class="box-header with-border">
            <h3 class="box-title">{{trans('admin.skills')}}</h3>
        </div>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @if(count($skills))
                            ...
                            <tr>
                                <td>{{$skill->id}}</td>
                                <td>{{@$skill->title[App::getLocale()] }}</td>
                                <td>{{$skill->slug}}</td>
                                <td>
                                ...
                                <td class="actions" width="200">
                                  <a class="btn btn-info" href="{{ action('Admin\SkillController@edit',$skill->id) }}" data-toggle="tooltip" title="{{trans('admin.edit')}}">
                                    {{trans('admin.edit')}}
                                  </a>
                                  {{ Form::open(array('url' => 'admin/skills/' . $skill->id,'style'=>'display:inline')) }}
