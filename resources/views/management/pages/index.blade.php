            ...
            <h3 class="box-title">
              ...
              @can('page-create')
                <span class="float-right">
                  <a class="btn btn-primary" href="{{ url('/admin/pages/create') }}">{{trans('admin.addpage')}}</a>
                </span>
              @endcan
            </h3>
        </div>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @if(count($pages))
                            ...
                            <tr>
                                <td>{{$page->id}}</td>
                                <td>{{@$page->title[App::getLocale()] }}</td>
                                <td>{{$page->slug}}</td>
                                <td>
                                      @if($page->is_active == 1)
                                ...
                                <td class="actions" width="200">
                                  <a class="btn btn-info" href="{{ action('Admin\PageController@edit',$page->id) }}" data-toggle="tooltip" title="{{trans('admin.edit')}}">
                                    {{trans('admin.edit')}}
                                  </a>
                                  {{ Form::open(array('url' => 'admin/pages/' . $page->id,'style'=>'display:inline')) }}
