...
<div class="col-md-12">
    ...
    <div class="box box-warning">
        <div class="box-header with-border mb-3">
            <h3 class="box-title">
              {{trans('admin.statuses')}}
              @can('status-create')
              <span class="float-right">
                <a class="btn btn-primary" href="{{ url('/admin/statuses/create') }}">{{trans('admin.addstatus')}}</a>
              </span>
              @endcan
            </h3>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @if(count($statuses))
                            ...
                            <tr>
                                <td>{{$status->id}}</td>
                                <td>{{@$status->title[App::getLocale()] }}</td>
                                <td>{{$status->slug}}</td>
                                <td>
                                ...
                                <td class="actions" width="200">
                                  <a class="btn btn-info" href="{{ action('Admin\StatusController@edit',$status->id) }}" data-toggle="tooltip" title="{{trans('admin.edit')}}">
                                    {{trans('admin.edit')}}
                                  </a>
                                  {{ Form::open(array('url' => 'admin/statuses/' . $status->id,'style'=>'display:inline')) }}
